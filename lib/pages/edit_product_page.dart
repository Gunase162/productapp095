...
class _EditProductPageState extends State<EditProductPage> {
  ...
  Future<String> getProductById() async {
    ...
    var url =
        Uri.parse('https://laravel-backend-cs.herokuapp.com/api/products/${widget.id}');
    ...
    return FutureBuilder(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            var paylond = jsonDecode(snapshot.data.toString())['product_type'];

            var ind = dropdownItems.indexWhere(
              (element) => element.value == paylond['product_type']
            );

            _name.text = paylond['product_name'];
            _price.text = paylond['price'].toString();
